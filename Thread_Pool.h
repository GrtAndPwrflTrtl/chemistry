...
{ // constructor (default thread count)
    num_threads=10;
...
{ // asynchronously process all incoming data
    ...
    {
        std::cout << "manager creation failed" << std::endl;
...
{ // manager thread
    ...
    //std::cout << "manager communication test: proc_func-threads=" << This->num_threads << std::endl;

    // spawn workers
    for (int x=1; x<=This->num_threads; x++)
    {
    ...
    for (int x=1; x<=This->num_threads; x++)
    {
        ...
        std::cout << "worker " << x << " removed" << std::endl;
    }
